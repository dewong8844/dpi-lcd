...
class SmDisplay:
	...
	def disp_weather(self):
		...
		lc = (255,255,255) 
		fn = "freesans"
		lg = 0.100	# Large Font Scale 0.125
		sm = 0.070	# Small Font Scale 0.075
		...
		# Time & Date
		font = pygame.font.SysFont( fn, int(ymax*lg), bold=1 )	# Regular Font
		sfont = pygame.font.SysFont( fn, int(ymax*sm), bold=1 )	# Small Font for Seconds
		...
		self.screen.blit( rtm1, (tp,3) )
		self.screen.blit( rtm2, (tp+tx1+3,5) )
		self.screen.blit( rtm3, (tp+tx1+tx2,3) )
		...
		txt = font.render( self.wind_speed+windSpeed, True, lc )
		...
		(ix,iy) = icon.get_size()
		icon2 = pygame.transform.smoothscale( icon, (int(ix*isc),int(iy*isc)) )
		(ix,iy) = icon2.get_size()
		if ( iy < 90 ):
			yo = (90 - iy) / 2 
		else: 
			yo = 0
		self.screen.blit( icon2, (xmax*wx-ix/2,ymax*(wy+gp*1.2)+yo-iyb) )
		...
		if ( iy < 90 ):
			...
			yo = 0
		self.screen.blit( icon2, (xmax*wx*3-ix/2,ymax*(wy+gp*1.2)+yo-iyb) )
		...
		if ( iy < 90 ):
			...
			yo = 0
		self.screen.blit( icon2, (xmax*wx*5-ix/2,ymax*(wy+gp*1.2)+yo-iyb) )
		...
		if ( iy < 90 ):
			...
			yo = 0
		self.screen.blit( icon2, (xmax*wx*7-ix/2,ymax*(wy+gp*1.2)+yo-iyb) )
